...
def BuscarAB(LISTA):
    rangoSeparacion = int((len(LISTA[0])/2))
    ValoresAB = []
...
def ObtenerMejores(Fitness, listaIndividuos):
    ...
    print('MejoresFit',mejor,'\n')
    print('Media ==>', media)
    print('Peor ==>', listaFail)
    #print(listaNueva[0])
    
    
    return mejor, media, listaFail
...
def Inicio(poblacion, tamIndiv, numDatos, x, y, Generaciones):
    ...
    while(MaxGeneracion < Generaciones):
        ...
        print('\nFitness\n',Fitness)
        tempLISTA = copy.copy(listaIndividuos)
        
        mejor, media, peor = ObtenerMejores(Fitness, tempLISTA)
